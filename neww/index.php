...
if (isset($_POST['save'])) {
	...
	if (isset($_GET['u_id'])) {
		...
		$db->update("class",$update_data);

	}else{
		$db->Insert("class",$_POST);	
	}
...
if (isset($_GET['d_id'])) {
	...
	$id = $_GET['d_id'];
	// $db->delete("user",array('id'=>$id, "condition"=>array('and name'=>'demo' , 'or email'=>'admin@gmail.com')));
	$db->delete("class",array('id'=>$id));
	header('location:index.php');
}
...
if (isset($_GET['u_id'])) {
	...
	$id = $_GET['u_id'];
	$data1 = $db->select("class",array('id'=>$id));

}


$data = $db->select("class");
